...
// Cross-company client list — only meaningful for admedika/mi_admin/
// super_admin (the server route silently ignores this for hr_client, who
// stay locked to their own single company everywhere else).
export async function getCompanyBreakdown(): Promise<CompanyBreakdownRow[]> {
  if (!auth?.currentUser) return [];
  try {
    const idToken = await auth.currentUser.getIdToken();
    const res = await fetch("/api/stats?include=companies", {
      headers: { Authorization: `Bearer ${idToken}` },
    });
    if (!res.ok) return [];
    const json = await res.json();
    return (json.companies as CompanyBreakdownRow[]) ?? [];
  } catch {
    return [];
  }
}
