...
// Per-company rollup for AdMedika's "Client List" and admin dashboards —
// headcount + assessment completion per company, cross-referenced against
// the `companies` directory for status/quota. Never exposes anything
// per-employee, only counts. Deliberately NOT available to hr_client (they
// only ever see their own single company via `company` scoping above) —
// this is for roles allowed to see across the whole client base.
...
async function computeCompanyBreakdown(): Promise<CompanyBreakdownRow[]> {
  const fdb = adminDb!;
  const usersSnap = await fdb.collection("users").where("role", "==", "employee").get();

  const byCompany = new Map<string, { uids: string[] }>();
  usersSnap.docs.forEach((d) => {
    const company = (d.data().company as string | undefined)?.trim();
    if (!company) return;
    if (!byCompany.has(company)) byCompany.set(company, { uids: [] });
    byCompany.get(company)!.uids.push(d.id);
  });

  const allResultsSnap = await fdb.collection("assessmentResults").get();
  const usersWithResults = new Set<string>();
  allResultsSnap.docs.forEach((d) => usersWithResults.add((d.data() as { uid: string }).uid));

  const bookingsSnap = await fdb.collection("counselingBookings").get();
  const bookingCountByUid = new Map<string, number>();
  bookingsSnap.docs.forEach((d) => {
    const bUid = (d.data() as { uid: string }).uid;
    bookingCountByUid.set(bUid, (bookingCountByUid.get(bUid) || 0) + 1);
  });

  const companiesSnap = await fdb.collection("companies").get();
  const companyMeta = new Map<string, { status?: string; employeeQuota?: number }>();
  companiesSnap.docs.forEach((d) => {
    const data = d.data();
    companyMeta.set((data.name as string)?.trim(), {
      status: data.status as string | undefined,
      employeeQuota: data.employeeQuota as number | undefined,
    });
  });

  const rows: CompanyBreakdownRow[] = [];
  byCompany.forEach(({ uids }, name) => {
    const withResults = uids.filter((u) => usersWithResults.has(u)).length;
    const sessions = uids.reduce((sum, u) => sum + (bookingCountByUid.get(u) || 0), 0);
    const meta = companyMeta.get(name);
    rows.push({
      name,
      employeeCount: uids.length,
      assessmentCompletionRate: uids.length > 0 ? Math.round((withResults / uids.length) * 100) : 0,
      counselingSessions: sessions,
      status: meta?.status as "active" | "inactive" | undefined,
      employeeQuota: meta?.employeeQuota,
    });
  });

  return rows.sort((a, b) => b.employeeCount - a.employeeCount);
}
...
export async function GET(req: NextRequest) {
  ...
  const includeCompanies = req.nextUrl.searchParams.get("include") === "companies";
  const max = Math.min(Math.max(Number(req.nextUrl.searchParams.get("max")) || 8, 1), 20);

  // Cross-company breakdown is never available to hr_client — they're
  // locked to their own company everywhere else in this route, so silently
  // ignore the param rather than 403 (avoids leaking "this param exists and
  // is being blocked" as a signal).
  if (includeCompanies && role !== "hr_client") {
    const cacheRef = adminDb.doc("statsCache/companyBreakdown");
    const cacheSnap = await cacheRef.get();
    const now = Date.now();
    const cached = cacheSnap.exists ? (cacheSnap.data() as { computedAt: number; rows: CompanyBreakdownRow[] }) : null;
    const isFresh = !!cached && now - cached.computedAt < CACHE_TTL_MS;
    const rows = isFresh && cached ? cached.rows : await computeCompanyBreakdown();
    if (!isFresh) await cacheRef.set({ computedAt: now, rows });
    return NextResponse.json({ companies: rows, cached: isFresh });
  }
