...
export default function LandingPage() {
  const { lang } = useLang();
  const { demoModeEnabled } = useDemoModeSetting();
  ...
  return (
    <div className="min-h-screen bg-background pb-20 lg:pb-0">
      ...
      {/* Desktop Header (Hidden on Mobile) - Floating Glassmorphism Navbar */}
      <header className="fixed inset-x-0 top-4 z-40 hidden justify-center px-4 lg:flex">
        <div className="flex w-full max-w-6xl items-center justify-between gap-6 rounded-full border border-border bg-white px-6 py-3 shadow-sm">
          ...
          <nav className="flex items-center gap-6">
            ...
            <Link href="#layanan" className="text-sm font-medium text-ink-soft hover:text-ink">
              Services
            </Link>
            <a href="https://mindfulness.id" target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-ink-soft hover:text-ink">
              Trainers
            </a>
            <a href="https://mindfulness.id" target="_blank" rel="noopener noreferrer" className="text-sm font-medium text-ink-soft hover:text-ink">
              Portfolio Gallery
            ...
            <Link href="#kontak" className="text-sm font-medium text-ink-soft hover:text-ink">
              Contact Us
            </Link>
          </nav>

          <nav className="flex shrink-0 items-center gap-4 border-l border-ink/10 pl-6">
            <Link href="/login" className="text-sm font-medium text-ink-soft hover:text-ink">
              {t("Masuk", lang)}
            </Link>
            <Link href="/register" className="rounded-full bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:bg-primary-soft">
              {t("Daftar", lang)}
            </Link>
            <LangToggle />
          </nav>
      ...
      {/* Hero Section */}
      <section className="relative overflow-hidden px-4 py-12 pt-20 sm:px-6 lg:py-24 lg:pt-32">
        <PulseHero />
      ...
      {/* Modules List - soft yellow full-bleed section */}
      <section id="layanan" className="bg-accent-soft/20 px-6 py-16 scroll-mt-24">
        <div className="mx-auto max-w-6xl">
          <div className="mb-10 text-center">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-accent px-2.5 py-0.5 text-[10px] font-bold uppercase tracking-wider text-primary">
              Layanan Kami
            </span>
            <h2 className="mt-3 font-display text-3xl font-bold tracking-tight text-ink sm:text-4xl">{t("modules_title", lang)}</h2>
            <span className="mt-3 inline-block h-1 w-20 rounded-full bg-accent" />
          </div>
          <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {modules.map((m, i) => (
              <div key={m.titleKey} className="rounded-2xl border border-accent/20 bg-white p-6 shadow-xs">
                <div className={`flex h-10 w-10 items-center justify-center rounded-full ${i % 2 === 0 ? "bg-primary text-white" : "bg-accent text-primary"}`}>
                  <m.icon size={18} />
                </div>
      ...
      {/* Keamanan & Kemitraan (Enterprise Trust) Section - now on top, yellow/amber background */}
      <section
        id="about"
        className="relative overflow-hidden bg-accent bg-cover bg-center px-6 py-16 scroll-mt-24"
        style={{ backgroundImage: "url('/hero-bg-yellow.png')" }}
      >
        ...
        <div className="max-w-2xl mx-auto mb-12">
          ...
          <span className="mt-3 block h-px w-full bg-primary/50" />
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          {/* HR Analytics Column */}
          <div className="relative group rounded-3xl border border-primary/10 bg-primary/5 p-6 backdrop-blur-sm transition-all hover:bg-primary/10">
            <BarChart3 size={22} className="text-primary" strokeWidth={1.75} />
          ...
          {/* Privacy Column */}
          <div className="relative group rounded-3xl border border-primary/10 bg-primary/5 p-6 backdrop-blur-sm transition-all hover:bg-primary/10">
            <Lock size={22} className="text-success" strokeWidth={1.75} />
            <h3 className="mt-4 font-display text-base font-bold text-primary">
          ...
          <div className="relative group rounded-3xl border border-primary/10 bg-primary/5 p-6 backdrop-blur-sm transition-all hover:bg-primary/10">
            <Users size={22} className="text-primary" strokeWidth={1.75} />
            <h3 className="mt-4 font-display text-base font-bold text-primary">
              Kemitraan AdMedika
            </h3>
            <p className="mt-2 text-xs leading-relaxed text-primary/70">
      ...
      {/* Contact Us Section - now below, deep purple background */}
      <section
        id="kontak"
        className="relative overflow-hidden bg-primary bg-cover bg-center px-6 py-16 scroll-mt-24"
      ...
      >
        ...
        <div className="relative mx-auto max-w-6xl">
          <div className="mb-8">
            ...
            <span className="mt-3 block h-px w-full bg-accent/60" />
          </div>

          <p className="max-w-2xl text-sm text-white/70 sm:text-base">
            Ada pertanyaan seputar layanan atau ingin memberi masukan? Hubungi kami kapan saja.
          </p>

          <div className="mt-6 space-y-2 text-sm text-white sm:text-base">
            <p>
              <span className="font-bold">Email us: </span>
              <a href="mailto:admin@mindfulness.id" className="text-accent underline decoration-accent/40 underline-offset-2 hover:decoration-accent">
                admin@mindfulness.id
              </a>
            </p>
            <p>
              <span className="font-bold">Text us : </span>
              <a href="https://wa.me/6281380948823" target="_blank" rel="noopener noreferrer" className="text-accent underline decoration-accent/40 underline-offset-2 hover:decoration-accent">
                0813-8094-8823
              </a>{" "}
              (WhatsApp)
            </p>
            <p>
              ...
              <a href="https://instagram.com/mindfulness.id" target="_blank" rel="noopener noreferrer" className="text-accent underline decoration-accent/40 underline-offset-2 hover:decoration-accent">
                @mindfulness.id
              </a>
            </p>
          </div>

          <div className="mt-6 text-sm text-white sm:text-base">
            <p className="font-bold">Official address :</p>
            <p className="text-white/70">Jl. Cemara Boulevard No. 53, Bogor, Jawa Barat</p>
          </div>

          <div className="mt-6 flex flex-wrap items-center gap-3">
            <a href="https://wa.me/6281380948823" target="_blank" rel="noopener noreferrer" aria-label="WhatsApp" className="flex h-9 w-9 items-center justify-center rounded-full border border-white/30 text-white transition-colors hover:bg-white/10">
              <Phone size={16} strokeWidth={1.75} />
            </a>
            <a href="mailto:admin@mindfulness.id" aria-label="Email" className="flex h-9 w-9 items-center justify-center rounded-full border border-white/30 text-white transition-colors hover:bg-white/10">
              <Mail size={16} strokeWidth={1.75} />
            </a>
            <a href="https://instagram.com/mindfulness.id" target="_blank" rel="noopener noreferrer" aria-label="Instagram" className="flex h-9 w-9 items-center justify-center rounded-full border border-white/30 text-white transition-colors hover:bg-white/10">
              <AtSign size={16} strokeWidth={1.75} />
            </a>
            <a href="#" aria-label="YouTube" className="flex h-9 w-9 items-center justify-center rounded-full border border-white/30 text-white transition-colors hover:bg-white/10">
              <Video size={16} strokeWidth={1.75} />
            </a>
            <a href="https://wa.me/6281380948823" target="_blank" rel="noopener noreferrer" aria-label="WhatsApp" className="flex h-9 w-9 items-center justify-center rounded-full border border-white/30 text-white transition-colors hover:bg-white/10">
              <MessageCircle size={16} strokeWidth={1.75} />
            </a>
            <a href="#" aria-label="TikTok" className="flex h-9 w-9 items-center justify-center rounded-full border border-white/30 text-white transition-colors hover:bg-white/10">
              <Music2 size={16} strokeWidth={1.75} />
            </a>
            <a href="#" aria-label="LinkedIn" className="flex h-9 w-9 items-center justify-center rounded-full border border-white/30 text-white transition-colors hover:bg-white/10">
          ...
          <div className="relative mt-6 overflow-hidden rounded-2xl border border-white/10">
            <a
              href="https://maps.google.com/?q=Jl.+Cemara+Boulevard+No.+53,+Bogor,+Jawa+Barat"
              target="_blank"
              rel="noopener noreferrer"
              className="absolute left-3 top-3 z-10 flex items-center gap-1.5 rounded-full bg-white px-3 py-1.5 text-xs font-semibold text-ink shadow-sm"
            >
              Buka di Maps
              <ArrowRight size={12} />
            </a>
            <iframe
              title="Lokasi Mindfulness Indonesia"
              src="https://maps.google.com/maps?q=Jl.+Cemara+Boulevard+No.+53,+Bogor,+Jawa+Barat&z=15&output=embed"
        ...
        <section className="border-t border-border bg-surface-sunk px-6 py-16">
          <div className="mx-auto max-w-6xl">
            <div className="mb-3 flex items-center gap-3">
              ...
              <h2 className="font-display text-3xl font-bold tracking-tight text-ink sm:text-4xl">{t("roles_title", lang)}</h2>
            </div>
            <span className="mb-4 inline-block h-1 w-20 rounded-full bg-primary" />
            <p className="mb-10 text-sm text-ink-soft">
              {t("roles_demo_notice", lang)}
            </p>
